    ...
    useEffect(() => {
        if (value && value.startDate && value.endDate) {
            if (dateIsSameOrBefore(value.startDate, value.endDate, "date")) {
                ...
                setInputText(
                    `${dateFormat(value.startDate, displayFormat, i18n)}${asSingle
                        ? ""
        ...
        return (
            <DatepickerContext.Provider value={contextValues}>
                <div className={cn("w-full", containerClassName)} ref={containerRef}>
                    <div className="shadow-sm border border-gray-300 px-1 py-0.5 bg-white rounded-lg">
                        <div className="flex flex-col lg:flex-row py-2">
                            {showShortcuts && <Shortcuts />}

                            <div
                                className={`flex items-stretch flex-col ${orientation === "horizontal" ? "md:flex-row space-y-4 md:space-y-0 md:space-x-1.5" : "space-y-4"
