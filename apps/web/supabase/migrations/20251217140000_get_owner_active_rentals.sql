-- Get Owner Active Rentals (Matching Dashboard Count)
-- Returns rentals with status Active, Approved, or Returned
...
RETURNS TABLE (
  ...
  listing_title text,
  listing_images text[], 
  listing_daily_price numeric,
  listing_hourly_price numeric
) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  SELECT 
    ...
    l.daily_price as listing_daily_price,
    l.hourly_price as listing_hourly_price
