-- Fix request_rental_extension RPC: chat lookup was using listing_id which is no longer
-- the unique key for chats. Chats are keyed by (owner_id, renter_id) pair only.
...
BEGIN
    ...
    -- 6. Get rental and listing details for system message
    SELECT 
        l.title,
        l.owner_id,
    ...
    -- 7. Get chat by owner+renter pair (NOT listing_id — chats are keyed by user pair)
