...
/**
 * Uploads return verification photos to Supabase storage
 * @param rentalId - UUID of the rental
 * @param files - Array of image files to upload (min 3, max 6)
 * @returns Promise<string[]> - Array of public URLs for uploaded images
 */
export async function uploadReturnPhotos(
    ...
    const uploadPromises = files.map(async (file, index) => {
        const timestamp = Date.now();
        const fileExt = file.name.split('.').pop();
        const fileName = `${rentalId}/return-${timestamp}-${index}.${fileExt}`;
    ...
    try {
        ...
        console.error('Return upload failed:', error);
        throw error;
    }
}

/**
 * Initiates the tool return process by calling the transition_to_returned RPC
 * @param rentalId - UUID of the rental
 * @param photoUrls - Array of uploaded return photo URLs
 * @returns Promise with success status and message
 */
export async function initiateReturn(
    ...
    const { data, error } = await supabase.rpc('transition_to_returned', {
        p_rental_id: rentalId,
        p_photo_urls: photoUrls
    });
