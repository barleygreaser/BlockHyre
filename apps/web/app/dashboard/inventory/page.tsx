...
const ListingThumbnail = ({ title, src }: { title: string; src?: string }) => {
  ...
  return (
    <span className="text-lg font-bold text-slate-400">{title.charAt(0)}</span>
  );
};
...
export default function InventoryPage() {
  ...
  useEffect(() => {
    document.title = "Inventory — BlockHyre";
  }, []);
  ...
  // Pre-calculate normalized fields to reduce time complexity from O(N*M) to O(N) during rapid state changes (e.g. search keystrokes)
  const normalizedInventory = useMemo(() => {
    return inventory.map((item) => ({
      ...item,
      _search_title: item.tool_title.toLowerCase(),
      _search_renter: item.current_renter_name?.toLowerCase() ?? "",
    }));
  }, [inventory]);
  ...
  const filteredInventory = useMemo(() => {
    ...
    return normalizedInventory.filter((item) => {
      ...
      if (!q) return true;
      return (
        item._search_title.includes(q) ||
        item._search_renter.includes(q)
      );
    });
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Track your tools, pricing, and availability.
          </p>
        </div>
        <Link href="/add-tool">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0">
          {STATUS_TABS.map((status) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold font-mono uppercase tracking-wider transition-all whitespace-nowrap ${
                statusFilter === status
            ...
            >
                ...
                >
                  <div
                    className={`h-2 w-2 rounded-full ${
                      item.status === "active"
                        ? "bg-emerald-500 animate-pulse-operational"
                        : item.status === "draft"
                          ? "bg-slate-400"
                          : "bg-slate-300"
                    }`}
              ...
              <div className="hidden md:flex col-span-2 items-center gap-1">
                ...
                <span className="font-bold text-slate-900 font-mono text-sm">
                  {item.avg_customer_rating > 0
                    ? item.avg_customer_rating.toFixed(1)
                    : "—"}
                </span>
              </div>
                  ...
                  <div className="text-xs font-mono">
                    ...
                    <div className="text-slate-400 mt-0.5">
                      ...
                      {dateFormatter.format(
                        new Date(item.current_rental_end_date!),
                      )}
                    </div>
                  </div>
