...
export function Hero() {
  ...
  const [suggestions, setSuggestions] = useState<
    { tool_name?: string; category_path?: string }[]
  >([]);
    ...
    const fetchSuggestions = async () => {
      ...
      try {
        const res = await fetch(
          `/api/fetch-suggestions?query=${encodeURIComponent(debouncedQuery)}&type=tool`,
        );
      ...
      try {
        ...
        ctx = gsap.context(() => {
          ...
          if (hasSeenAnimation) {
            ...
            gsap.set(
              [".hero-line-1", ".hero-line-2", ".hero-subtitle", ".hero-cta"],
              {
                y: 0,
                opacity: 1,
                skewY: 0,
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        ...
        <video
          ...
          loop
          playsInline
          poster="/mobile_hero_video_still.png"
        ...
        {/* Desktop video — shown at md breakpoint and above */}
        <video
          autoPlay
          muted
          loop
      ...
      {/* Content */}
      <div
        className="container relative z-40 mx-auto px-4 md:px-8 mt-12 md:mt-24"
        ref={headlineRef}
      >
        <div className="max-w-3xl space-y-6 md:space-y-8">
          ...
          <div>
            <h1 className="flex flex-col gap-2 md:gap-3">
              <span
                className="hero-line-1 block text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-eerie-black leading-[1.1] tracking-[-0.02em]"
                style={{
                  textShadow:
                    "0 1px 3px rgba(255,255,255,0.8), 0 0 20px rgba(255,255,255,0.4)",
                }}
              >
                Rent what you need.
              </span>
              <span
                className="hero-line-2 block text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-electric-indigo leading-[1.1] tracking-[-0.02em]"
                style={{
                  textShadow:
                    "0 1px 3px rgba(255,255,255,0.9), 0 0 30px rgba(255,255,255,0.5), 0 2px 8px rgba(0,0,0,0.15)",
                }}
              >
          ...
          <p className="hero-subtitle text-base md:text-lg text-eerie-black/70 leading-relaxed max-w-xl font-sans font-normal">
            High-quality gear from neighbors you trust. Nearby and ready when
            you are.
          </p>
          ...
          <div
            ...
            className="hero-cta relative w-full max-w-2xl z-50"
          >
            <div className="w-full bg-signal-white/80 backdrop-blur-md p-2 rounded-2xl border border-opal/50 shadow-xl flex gap-2 items-center group focus-within:bg-white focus-within:border-electric-indigo transition-all">
              <div className="hidden sm:flex items-center justify-center p-2.5">
                <svg
                  ...
                  className="text-eerie-black/40 group-focus-within:text-electric-indigo transition-colors"
                >
                ...
                <div className="max-h-72 overflow-y-auto">
                  {suggestions.map((item, idx) => (
                    <button
                      ...
                      onClick={() =>
                        handleSelectSuggestion(item.tool_name || "")
                      }
                    >
          ...
          <div className="hero-cta flex flex-wrap items-center gap-3 pt-4">
            <span className="text-sm font-medium text-eerie-black/70">
              Popular:
            </span>
            {["Power Tools", "Lawn & Garden", "Painting", "Plumbing"].map(
              (category) => (
                <Link
                  key={category}
          ...
          <div className="hero-cta flex items-center gap-3 pt-4">
            ...
            <p className="text-sm text-eerie-black/60 font-medium">
              Trusted by{" "}
              <span className="text-eerie-black font-bold">2,000+</span> homes
              in your block
            </p>
          </div>
        </div>
      ...
      >
        <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-eerie-black/60 group-hover:text-electric-indigo transition-colors">
          Scroll
        </span>
