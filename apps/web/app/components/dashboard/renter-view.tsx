...
export function RenterDashboardView() {
    ...
    // Performance improvement: pre-calculate derived counts in O(N) instead of O(N * 2)
    // and memoize the result to avoid recalculations on every render.
    const { urgentCount, overdueCount } = useMemo(() => {
        let urgent = 0;
        let overdue = 0;
        for (let i = 0; i < activeRentals.length; i++) {
            const status = activeRentals[i].dashboard_status;
            if (status === 'overdue') {
                overdue++;
                urgent++;
