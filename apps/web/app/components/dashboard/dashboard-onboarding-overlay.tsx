...
const RENTER_TOUR_STEPS_ALL = [
    {
        ...
        mobileSkip: true,
        title: "Renter Mode",
        description: "Your default view. Manage the tools you are currently renting, pending requests, and track your history all in one place.",
        icon: null
    },
    {
        ...
        mobileSkip: true,
        title: "Owner Mode",
        description: "Click here to switch to the Command Center. Manage your listed tools, approve bookings, and track your rental earnings instantly.",
        icon: null
    },
    // Mobile-only replacement for the two steps above
    {
        ...
        description: "Tap the hamburger menu icon highlighted above to open navigation. From there you can instantly switch between Renter and Owner mode.",
        icon: "menu"
    },
    {
        ...
        description: "Keep an eye on these indicators. We'll alert you here if any of your rentals are overdue or need attention today.",
        icon: null
    },
    {
        ...
        description: "Rentals live and die in the DMs. All inquiries and negotiations with a specific neighbor happen in a single, persistent thread here in your Messages.",
        icon: null
    },
    {
        ...
        description: "Ready to build? Click here to find the tools you need in your neighborhood.",
        icon: null
    }
] as const;
...
const OWNER_TOUR_STEPS_ALL = [
    {
        ...
        description: "Welcome to the Owner view! This is your control room for managing rentals, tracking your earnings, and organizing your fleet.",
        icon: null
    },
    {
        selector: null,
        ...
        mobileOnly: true,
        title: "Command Center",
        description: "Welcome to the Owner view! Open the menu here to find your owner-specific links like My Fleet and Transactions.",
        icon: "menu"
    },
    {
        ...
        description: "Monitor your payouts and connect your bank account via Stripe to receive payments securely.",
        icon: null
    },
    {
        ...
        description: "Keep track of all your listed tools, update their availability, and adjust your daily pricing here.",
        icon: null
    }
] as const;

type Step = typeof RENTER_TOUR_STEPS_ALL[number] | typeof OWNER_TOUR_STEPS_ALL[number];
...
function resolveElement(step: Step): HTMLElement | null {
    ...
    const primarySelector = isMobile
        ? ('mobileSelector' in step ? step.mobileSelector : step.selector)
        : step.selector;
...
function calcPopoverPosition(rect: DOMRect): { top: number; left: number } {
    ...
    if (rect.left < 40 && rect.right + PADDING + popoverWidth < window.innerWidth) {
        const left = rect.right + PADDING;
        // Vertically align it roughly with the item, clamping so it doesn't bleed off screen
        const top = Math.max(16, Math.min(rect.top - PADDING, window.innerHeight - estimatedHeight - 16));
        return { top, left };
    }
...
export function DashboardOnboardingOverlay() {
    ...
    useEffect(() => {
        if (!pathname?.includes("/dashboard")) { setIsOpen(false); return; }
        if (userProfileLoading || !user || !userProfile?.neighborhoodId) return;

        const storageKey = activeRole === 'owner' ? "blockhyre_owner_onboarding_v1" : "blockhyre_dashboard_onboarding_v1";
        ...
        const interval = setInterval(() => {
            ...
            const noElementNeeded = !('selector' in firstStep && firstStep.selector) && !('mobileSelector' in firstStep && firstStep.mobileSelector);
    ...
    const updateTarget = useCallback(() => {
        ...
        if (el) {
            ...
            const isOffscreen = !isFixedAtTop && (rect.bottom > window.innerHeight - 80 || rect.top < 80);
            if (isOffscreen) {
    ...
    const rect = targetRect ?? (isMobile ? null : new DOMRect(
    ...
    return (
        ...
        >
            ...
            <div className={`absolute inset-0 cursor-default transition-opacity duration-500 ${isMounted ? 'opacity-100' : 'opacity-0'}`} />
            ...
            {!isMobile && (
                <div
                    ...
                    style={{ top: desktopPos.top, left: desktopPos.left, width: popoverWidth }}
                    onKeyDown={(e) => {
                        if (e.key !== 'Tab') return;
            ...
            <div className="p-6">
                ...
                <p className="text-center text-[10px] text-slate-300 font-mono mt-3">
                    {isMobile
                        ...
                        : <>Press <kbd className="bg-slate-100 text-slate-500 px-1 rounded">↵</kbd> to advance · <kbd className="bg-slate-100 text-slate-500 px-1 rounded">Esc</kbd> to skip</>
                    }
                </p>
            </div>
        </>
