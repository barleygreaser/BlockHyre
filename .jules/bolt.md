...
## 2025-03-03 - Optimize List Filtering and Derived Counts in Render loops (Update)
**Learning:** Performing multiple array `.filter()` traversals to derive category counts or recalculating filtered lists on every render (without `useMemo`) creates an O(N * M) performance bottleneck, especially when loop-invariant operations like string normalization (`toLowerCase()`) are placed inside the loop. When refactoring to use `useMemo`, helper functions must also be stabilized with `useCallback` to prevent unnecessary re-computations.
**Action:** Always wrap derived lists in `useMemo`, hoist loop-invariant operations like search normalization outside the loop, and use an O(N) array reduction/traversal to pre-calculate category counts once rather than filtering repeatedly. Also, when extracting filtering logic to a helper function within a component, use `useCallback` to prevent unnecessary invalidation of `useMemo` dependencies.
