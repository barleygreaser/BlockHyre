...
## 2024-05-24 - Array Iteration and Memoization
**Learning:** Found O(N*M) performance bottlenecks when deriving category counts directly in the render loop by repeatedly calling `.filter(...).length` on large arrays.
**Action:** Use a single `useMemo` wrapped O(N) `for...of` traversal to calculate multiple derived state variables simultaneously, which prevents redundant calculations and array allocations on every render.
