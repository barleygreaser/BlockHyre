...
## 2024-05-15 - localeCompare in TypeaheadInput sorting
**Learning:** `apps/web/app/components/ui/typeahead-input.tsx` uses `localeCompare` to sort `categories` in `fetchSuggestions` logic whenever `debouncedValue` changes. However, the `categories` array passed as a prop from `useMarketplace` is already sorted using `localeCompare` when fetched and cached. Therefore, doing an O(N log N) `localeCompare` sort on every keystroke in `TypeaheadInput` is completely redundant and causes unnecessary performance overhead during typing. We can simply rely on the pre-sorted order and just filter the array, or if we want to prioritize matches that *start* with the query, we can use an O(N) approach to split the array into `startsWith` matches and `includes` matches and then concatenate them, which preserves the original alphabetical sort order from the pre-sorted array!
**Action:** When working with pre-sorted arrays (like categories), avoid chaining `.sort()` with `localeCompare` after `.filter()` just to prioritize matches. Instead, partition the filtered array into groups (e.g., `startsWith` and `includes`) using a single O(N) pass, and concatenate them. This naturally preserves the pre-sorted alphabetical order within each group without invoking the expensive `localeCompare` sort.
