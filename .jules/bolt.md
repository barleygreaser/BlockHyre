...
## 2026-05-25 - Optimize FlatList in favorites.tsx
**Learning:** Found that \`renderItem\` and \`renderEmpty\` along with \`handleRemoveFavorite\` inside \`apps/mobile/app/favorites.tsx\` lacked \`useCallback\`, leading to unnecessary re-renders in a FlatList. Memoizing these and pulling \`keyExtractor\` out of component maintains referential stability and improves list performance.
**Action:** Always wrap FlatList render handlers and list helpers in \`useCallback\` and move static extractors outside the component definition to ensure referential stability and avoid unnecessary re-renders.
