...
## 2025-03-02 - Array Filtering Optimization
**Learning:** Using multiple `.filter(...).length` calls on the same array to calculate derived status counts inside a React component render function causes unnecessary redundant array traversals on every render.
**Action:** Replace multiple `.filter()` passes with a single `useMemo`-wrapped `O(N)` loop that counts all required metrics simultaneously to improve rendering performance.
