...
## 2025-03-02 - Optimize List Filtering and Derived Counts in Render loops
**Learning:** Performing multiple array `.filter()` traversals to derive counts (e.g. `urgentCount`, `overdueCount`) recalculates them on every render.
**Action:** Use `useMemo` with an O(N) `reduce` traversal to pre-calculate all derived counts in a single pass.
