...
FROM harvest_cat
WHERE NOT EXISTS (SELECT 1 FROM public.listings WHERE title = 'Harvest Right Freeze Dryer');
...
SELECT 
    ...
    (SELECT id FROM owner_user)
FROM power_cat
WHERE NOT EXISTS (SELECT 1 FROM public.listings WHERE title = 'Makita Impact Drill Kit');
