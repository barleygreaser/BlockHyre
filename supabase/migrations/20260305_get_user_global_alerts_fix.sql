...
BEGIN
    ...
    -- Owner overdue & due today (mirroring renter logic)
    SELECT count(*) INTO v_owner_overdue
    FROM rentals r
    JOIN listings l ON r.listing_id = l.id
    ...
    SELECT count(*) INTO v_owner_due_today
    FROM rentals r
    JOIN listings l ON r.listing_id = l.id
    WHERE l.owner_id = p_user_id 
      AND r.status ILIKE ANY(ARRAY['active', 'approved'])
    ...
    v_result := jsonb_build_object(
        'owner_action_required', coalesce(v_owner_pending_requests, 0) + coalesce(v_owner_needs_inspection, 0) + coalesce(v_owner_pending_extensions, 0) + coalesce(v_owner_overdue, 0) + coalesce(v_owner_due_today, 0),
...
EXCEPTION WHEN undefined_table THEN
    ...
    v_result := jsonb_build_object(
        'owner_action_required', coalesce(v_owner_pending_requests, 0) + coalesce(v_owner_needs_inspection, 0) + coalesce(v_owner_overdue, 0) + coalesce(v_owner_due_today, 0),
        'renter_action_required', coalesce(v_renter_overdue, 0) + coalesce(v_renter_due_today, 0),
        'renter_overdue_count', coalesce(v_renter_overdue, 0)
    );
    RETURN v_result;
